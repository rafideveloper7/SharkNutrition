...
export default function AllProducts() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/products/getAllProducts`, {
        credentials: "include",
      });
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      formData.append(
        "discountPercent",
        Number(currentProduct.discountPercent)
      );
      ...
      currentProduct.existingGallery.forEach((url) =>
  ...
  if (loading)
    return (
      <div className="text-blue p-4 fles justify-center items-center">
        Loading...
      </div>
    );
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => {
            ...
            return (
              <div
                key={product._id}
                className="bg-gray-800 rounded-lg p-4 text-white shadow-lg"
              >
                <img
                ...
                <p className="text-gray-300 text-base lg:text-lg mb-2">
                  Quantity:{" "}
                  <span className="text-blue-300">{product.quantity || 0}</span>
                </p>

                <p className="text-gray-300 text-sm lg:text-base mb-2">
                  Servings: {product.servings?.join(", ")}
                </p>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-2 mt-1">
            ...
            <label className="block text-gray-300 mb-1 text-sm">Quantity</label>
            <Input
              type="number"
            ...
            />
            <label className="block text-gray-300 mb-1 text-sm">Weight</label>
            <Input
              value={currentProduct.weight}
              onChange={(e) =>
                setCurrentProduct({ ...currentProduct, weight: e.target.value })
              }
              placeholder="Weight"
            />
                    ...
                    <button
                      ...
                      onClick={() =>
                        setCurrentProduct({
                          ...currentProduct,
                          existingGallery:
                            currentProduct.existingGallery.filter(
                              (_, i) => i !== idx
                            ),
                        })
                      }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setCurrentProduct({
                    ...currentProduct,
                    gallery: [...currentProduct.gallery, ...e.target.files],
                  })
                }
                className="hidden"
              />
                  ...
                  <div key={idx} className="relative w-20 h-20">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setCurrentProduct({
                          ...currentProduct,
                          gallery: currentProduct.gallery.filter(
                            (_, i) => i !== idx
                          ),
                        });
                      }}
