...
export default function AddProducts() {
  ...
  const handleChange = (e, idx, type) => {
    if (e.target.name === "gallery") {
      const files = Array.from(e.target.files || []); // FileList to Array safely
      setFormData({
        ...formData,
        gallery: [...(formData.gallery || []), ...files], // ensure previous gallery is array
      });
      return; // early return
    } else if (type === "flavor") {
    ...
    if (["price", "quantity", "discountPercent"].includes(name)) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <button
                type="button"
                onClick={() =>
                  setFormData({ ...formData, flavor: [...formData.flavor, ""] })
                }
